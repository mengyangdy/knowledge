<template>
  <div class="blog-search" v-if="openSearch" data-pagefind-ignore="all">
    <div class="nav-search-btn-wait" @click="searchModal=true">
      ...
      <span v-if="!isMinimized" class="metaKey">
        {{ metaKey }} K
      </span>
    </div>
    <Command.Dialog :visible="searchModal" theme="algolia">
      <template #header>
        <Command.Input v-model:value="searchWords" :placeholder="searchConfig?.placeholder || '请输入要搜索的内容'"/>
      </template>
      <template #body>
        <div class="search-dialog">
          <Command.List>
            ...
            <Command.Group v-else :heading="headingText">
              <Command.Item v-for="item in searchResult" :data-value="item.route" @select="handleSelect">
                <div class="link">
                  <div class="title">
                    <span>
                      {{ item.meta.title }}
                    ...
                    <span class="date">
                      {{ formatDate(item.meta.date, 'yyyy-MM-dd') }}
                    </span>
                  </div>
      ...
      <template #footer v-if="searchResult.length">
        <div class="command-palette-logo">
          <a v-if="openSearch === 'pagefind'" href="https://github.com/cloudcannon/pagefind" target="_blank"
...
const headingText = computed(() => {
  return searchConfig.heading ? searchConfig.heading.replace(
    /\{\{searchResult\}\}/,
    searchResult.value.length
  ) : `共：${searchResult.value.length} 个搜索结果`
})
const openSearch = computed(() =>
  searchConfig instanceof Object ? searchConfig.mode ?? true
    : searchConfig ?? true
)

const addInlineScript = () => {
  const scriptText = `import('${withBase('/_pagefind/pagefind.js')}').then((module)=>{
  window.__pagefind__ = module
  })
  .catch(()=>{
  console.log('not load /__pagefind/pagefind.js')
  })
  `
