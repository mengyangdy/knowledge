...
最近在看法uniapp项目的时候，接收过来的时候unpackage下面的dist文件夹也在git版本控制中，每次运行在模拟器都会生成新的文件，所以想把它在版本控制中取消掉，我们可以执行以下的命令：

```bash
git rm file_path --cached
```

这个命令是把某一个文件从版本控制中删除，如果是一个文件夹就不行了需要加上-r的参数

```bash
git rm -r folder_path --cached
```

如果需要删除的文件夹太多了，我们就可以先移除所有的文件的版本控制，然后再添加：
...
有时候我们修改过一些文件之后又不想通过add和commit提交他们，我们就可以这些修改的文件给还原了，如果是对已经在git版本控制中的文件进行修改的话我们可以用以下的命令：

```bash
git checkout .
```

但是对于新建的文件这样就不能恢复了，需要用以下的命令：

```bash
# 查看有哪些文件是未被git跟踪的 只查看不删除
git clean -n 
# 删除所有未被跟踪的文件
git clean -f
# 删除某一个未被跟踪的文件
